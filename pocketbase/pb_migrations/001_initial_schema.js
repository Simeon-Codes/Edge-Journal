 ...
 * App       : EDGE Journal
 * Target    : PocketBase v0.23+ (tested against v0.28.2)
 *
 * Field properties are FLAT on the field object — no nested `options: {}`.
 * This matches the official docs at https://pocketbase.io/docs/js-collections/
 *
...
var OWN_ROW  = "user = @request.auth.id";
...
migrate(function(app) {
  ...
  var profiles = new Collection({
    ...
    fields: [
      ...
      { name: "display_name", type: "text",     required: true,  max: 60 },
      { name: "timezone",     type: "text",     required: false, max: 60 },

      { name: "tier",                   type: "number",  required: true,  min: 0, max: 5 },
      { name: "subscription_status",    type: "select",  required: true,
        values: ["trial","active","past_due","canceled","paused"], maxSelect: 1 },
      { name: "trial_ends_at",          type: "date",    required: false },
      { name: "stripe_customer_id",     type: "text",    required: false, max: 100 },
      { name: "stripe_subscription_id", type: "text",    required: false, max: 100 },

      { name: "investor_password_hash", type: "text",    required: false, max: 255 },
      { name: "investor_link_enabled",  type: "bool",    required: true },

      { name: "theme",             type: "select", required: true,
        values: ["dark","light","system"], maxSelect: 1 },
      ...
      { name: "default_currency",  type: "text",   required: false, max: 10 },
    ],
  });
  app.save(profiles);
  ...
  var mt5Accounts = new Collection({
    ...
    fields: [
      { name: "user",          type: "relation", required: true,
        collectionId: "_pb_users_auth_", cascadeDelete: true, maxSelect: 1 },
      { name: "account_label", type: "text",    required: true,  max: 60 },
      { name: "mt5_login",     type: "text",    required: true,  max: 30 },
      { name: "broker",        type: "text",    required: false, max: 100 },
      { name: "server",        type: "text",    required: false, max: 100 },
      { name: "api_key_hash",  type: "text",    required: true,  max: 255 },
      { name: "currency",      type: "text",    required: false, max: 10 },
  ...
  var mt5Col = app.findCollectionByNameOrId("mt5_accounts");
  ...
  var trades = new Collection({
    ...
    fields: [
      ...
      { name: "mt5_account", type: "relation", required: false,
        collectionId: mt5Col.id,          cascadeDelete: false, maxSelect: 1 },

      { name: "mt5_ticket",  type: "text",   required: false, max: 30 },
      { name: "source",      type: "select", required: true,
        values: ["manual","mt5_ea","mt5_import"], maxSelect: 1 },

      { name: "opened_at",   type: "date",   required: true },
      { name: "closed_at",   type: "date",   required: false },

      { name: "pair",        type: "text",   required: true,  max: 20 },
      { name: "direction",   type: "select", required: true,
        values: ["LONG","SHORT"], maxSelect: 1 },
      { name: "session",     type: "text",   required: false, max: 30 },
      { name: "setup",       type: "text",   required: false, max: 60 },
      ...
      { name: "emotions",      type: "text",  required: false, max: 30 },
      { name: "followed_plan", type: "bool",  required: false },
      { name: "mistakes",      type: "text",  required: false, max: 1000 },
      { name: "notes",         type: "text",  required: false, max: 5000 },
      { name: "tags",          type: "json",  required: false },
      { name: "grade",         type: "text",  required: false, max: 5 },

      { name: "chart_images",  type: "file",  required: false,
        maxSelect: 2, maxSize: 5242880,
  ...
  var journalEntries = new Collection({
    ...
    fields: [
      ...
      { name: "content",     type: "text",   required: false, max: 10000 },
      { name: "market_bias", type: "select", required: false,
        values: ["bullish","bearish","neutral","ranging"], maxSelect: 1 },
      { name: "mood",        type: "number", required: false, min: 1, max: 10 },
      { name: "tags",        type: "json",   required: false },
    ],
  });
  ...
  var investorLinks = new Collection({
    ...
    fields: [
      ...
      { name: "label",          type: "text",   required: false, max: 60 },
      { name: "is_active",      type: "bool",   required: true },
  ...
  var usageLogs = new Collection({
    ...
    fields: [
      ...
      { name: "trades_count",  type: "number", required: true, min: 0 },
      { name: "total_lots",    type: "number", required: true, min: 0 },
      { name: "tier_at_time",  type: "number", required: true, min: 0, max: 5 },
  ...
  var syncLogs = new Collection({
    ...
    fields: [
      { name: "mt5_account", type: "relation", required: true,
        collectionId: mt5Col.id, cascadeDelete: true, maxSelect: 1 },
      { name: "event_type",  type: "select",   required: true,
        values: ["connect","disconnect","trade_open","trade_close","trade_modify","full_sync","error"],
        maxSelect: 1 },
      { name: "payload",     type: "json",  required: false },
      { name: "error_msg",   type: "text",  required: false, max: 500 },
  ...
  var names = [
    "sync_logs", "usage_logs", "investor_links",
    "journal_entries", "trades", "mt5_accounts", "profiles",
  ];
