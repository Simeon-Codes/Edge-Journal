...
export function Analytics({ trades, stats }) {
  ...
  if (!trades.length) return <div style={{ color:t.textMuted, textAlign:'center', padding:60 }}>Log trades to see analytics</div>;

  const TT = ({ active, payload }) => {
    if (!active||!payload?.length) return null;
    return <div style={{ background:t.bgCard, border:`1px solid ${t.border}`, borderRadius:8, padding:'8px 12px', fontSize:11 }}><div style={{ color:pnlColor(payload[0]?.value,t), fontWeight:700 }}>${payload[0]?.value}</div></div>;
  };
